...
class _MapScreenState extends State<MapScreen> with TickerProviderStateMixin {
  ...
  void initState() {
    ...
    // 確保 Store 已初始化 (MapCubit initLocation is called in app.dart)
    ...
    return Scaffold(
      body: BlocBuilder<OfflineMapCubit, OfflineMapState>(
        builder: (context, offlineState) {
          final isStoreReady = offlineState is OfflineMapLoaded && offlineState.isStoreReady;
          final packageName = offlineState is OfflineMapLoaded ? offlineState.packageName : '';

          return BlocBuilder<MapCubit, MapState>(
            builder: (context, mapState) {
              // Extract data from MapState safely
              final trackPoints = mapState is MapLoaded ? mapState.trackPoints : <LatLng>[];
              final currentLocation = mapState is MapLoaded ? mapState.currentLocation : null;
              final currentHeading = mapState is MapLoaded ? mapState.currentHeading : null;
              final isLoading = mapState is MapLoaded && mapState.isLoading;
                  ...
                  FlutterMap(
                    ...
                    children: [
                      ...
                      if (trackPoints.isNotEmpty)
                        PolylineLayer(
                          polylines: [Polyline(points: trackPoints, color: Colors.blue, strokeWidth: 4.0)],
                        ),
                              ...
                              child: Stack(
                                ...
                                children: [
                                  if (currentHeading != null)
                                    Transform.rotate(
                                      angle: (currentHeading * (3.14159 / 180) * -1),
                              ...
                              ? Column(
                                  ...
                                  children: [
                                    ...
                                    FloatingActionButton(
                                      ...
                                      onPressed: () async {
                                        try {
                                          await context.read<MapCubit>().loadGpxFile();
                                    ...
                                    if (!kIsWeb) ...[
                                      ...
                                      FloatingActionButton(
                                        ...
                                        onPressed: () => _showDownloadDialog(context, _mapController),
                                        tooltip: '下載離線地圖',
                                    ...
                                    FloatingActionButton(
                                      ...
                                      onPressed: () {
                                        ...
                                        if (s is MapLoaded && s.currentLocation != null) {
                                          _animatedMapMove(
                                            LatLng(s.currentLocation!.latitude, s.currentLocation!.longitude),
                                            15.0,
                                          );
                                        } else {
                                          context.read<MapCubit>().initLocation();
      ...
      builder: (ctx) => AlertDialog(
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () {
              Navigator.pop(ctx);
              setState(() => _previewBounds = null); // 開始下載後清除紅框
  ...
  Future<void> _startDownload(LatLngBounds bounds) async {
    ...
    try {
      // 觸發 Cubit 下載
      await offlineCubit.downloadRegion(
        bounds: bounds,
        minZoom: 12,
        maxZoom: 20,
