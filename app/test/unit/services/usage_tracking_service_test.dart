...
void main() {
  ...
  group('UsageTrackingService Tests', () {
    ...
    test(
      'Given web and skip on non-Web, When calling UsageTrackingService Tests, Then start() should send heartbeat',
      () async {
        // 安排
    ...
    test(
      'Given UsageTrackingService Tests, When executing, Then start() without userId should send guest heartbeat',
      () async {
        // 安排
        Map<String, dynamic>? capturedData;
    ...
    test('Given UsageTrackingService Tests, When 執行測試, Then stop() 應取消定時器', () {
      // 執行 & 驗證 - 不應拋出異常
      expect(() => service.stop(), returnsNormally);
      expect(() => service.stop(), returnsNormally);
    });

    test('Given UsageTrackingService Tests, When 執行測試, Then dispose() 應釋放資源', () {
      // 執行 & 驗證
      expect(() => service.dispose(), returnsNormally);
    });

    test('Given UsageTrackingService Tests, When 執行測試, Then 建構子應接受自訂 ApiClient', () {
      // 執行
