exports.handler = async function (event, context) {
  ...
  try {
    ...
    const targetLocationName =
      event.queryStringParameters.locationName || "向陽山";
    ...
    if (jsonData?.cwaopendata?.dataset?.locations?.location) {
      ...
      const targetLoc = locations.find(
        (l) => l.locationName === targetLocationName
      );
      ...
      if (targetLoc) {
        ...
        console.warn(
          `Location ${targetLocationName} not found in API response.`
        );
        ...
        return {
          ...
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            error: `Location '${targetLocationName}' not found in weather data.`,
          }),
        };
      }
    }
